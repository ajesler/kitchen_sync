	...
	void handle_hash_response(TableJob &table_job) {
		...
		if (hash_result.our_last_key != last_key) {
			...
			if (hash_result.estimated_rows_in_range == UNKNOWN_ROW_COUNT) {
				...
				table_job.ranges_to_check.push_back(make_tuple(hash_result.our_last_key, last_key, UNKNOWN_ROW_COUNT, rows_to_scan_forward_next(rows_to_hash, match, hash_result.our_row_count, hash_result.our_size)));
			} else {
				// we're hunting errors, do that first
				size_t rows_remaining = hash_result.estimated_rows_in_range - hash_result.our_row_count;
				table_job.ranges_to_check.push_front(make_tuple(hash_result.our_last_key, last_key, rows_remaining, rows_remaining));
			}
		}

		if (!match) {
			// the part that we checked has an error; decide whether it's large enough to subdivide
			handle_hash_not_matching(table_job, prev_key, hash_result.our_last_key, hash_result.our_row_count, hash_result.our_size);
		}
	}

	inline void handle_hash_not_matching(TableJob &table_job, const ColumnValues &prev_key, const ColumnValues &last_key, size_t our_row_count, size_t our_size) {
		// the part that we checked has an error; decide whether it's large enough to subdivide
		if (our_row_count > 1 && our_size > target_minimum_block_size) {
			// yup, queue it up for another iteration of hashing
			table_job.ranges_to_check.push_front(make_tuple(prev_key, last_key, our_row_count, decide_rows_to_hash(our_row_count)));
		} else {
			// not worth subdividing the range any further, queue it to be retrieved
			table_job.ranges_to_retrieve.push_back(make_tuple(prev_key, last_key));
		}
	}
