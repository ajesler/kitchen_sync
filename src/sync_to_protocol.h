...
struct SyncToProtocol {
	...
	void sync_table(const Table &table) {
		...
		if (worker.verbose) {
			...
			cout << time(nullptr) << " starting " << table.name << endl << flush;
		}
		...
		while (true) {
			...
			if (!table_job.ranges_to_retrieve.empty()) {
				...
				if (worker.verbose > 1) cout << time(nullptr) << " <- rows " << table_job.table.name << ' ' << values_list(client, table_job.table, prev_key) << ' ' << values_list(client, table_job.table, last_key) << endl;
				...
				if (worker.verbose > 1) cout << time(nullptr) << " <- hash " << table_job.table.name << ' ' << values_list(client, table_job.table, prev_key) << ' ' << values_list(client, table_job.table, last_key) << ' ' << rows_to_hash << endl;
				...
				if (worker.verbose > 1) cout << time(nullptr) << " -> hash " << table_job.table.name << ' ' << values_list(client, table_job.table, prev_key) << ' ' << values_list(client, table_job.table, last_key) << ' ' << their_row_count << (match ? " matches" : " doesn't match") << endl;
		...
		if (worker.verbose) {
			...
			unique_lock<mutex> lock(sync_queue.mutex);
			cout << time(nullptr) << " finished " << table.name << " in " << (now - started) << "s using " << hash_commands << " hash commands and " << rows_commands << " rows commands changing " << row_replacer.rows_changed << " rows" << endl << flush;
		}
	...
	void establish_range(TableJob &table_job) {
		...
		if (worker.verbose > 1) cout << time(nullptr) << " <- range " << table_job.table.name << endl;
		...
		if (worker.verbose > 1) cout << time(nullptr) << " -> range " << table_job.table.name << ' ' << values_list(client, table_job.table, their_first_key) << ' ' << values_list(client, table_job.table, their_last_key) << endl;
	...
	void handle_rows_command(const Table &table, RowReplacer<DatabaseClient> &row_replacer) {
		...
		if (worker.verbose > 1) cout << time(nullptr) << " -> rows " << table.name << ' ' << values_list(client, table, prev_key) << ' ' << values_list(client, table, last_key) << endl;
